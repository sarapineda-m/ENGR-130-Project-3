...
def edit_account_info(username):
    ...
    win = GraphWin("Edit Info", 450, 800)
    center_window(win, 450, 800)
    win.setCoords(0, 0, 450, 800)
    win.setBackground("#cfb991")
    x = 225
    ...
    user_box, user_text = draw_field(600, "Username:", user["username"])
    pass_box, pass_text = draw_field(490, "Password:", user["password"])
    vehicle_box, vehicle_text = draw_field(380, "Vehicle Number:", user["vehicle"])
    verify_box, verify_text = draw_field(270, "Verification Code:", user["verification"])
    identity_box, identity_text = draw_field(160, "Identity:", user["identity"])

    update_btn = Rectangle(Point(100, 100), Point(350, 140))
    update_btn.setFill("#0f9d58")
    update_btn.draw(win)
    Text(update_btn.getCenter(), "Update Info").draw(win)

    back_btn = Rectangle(Point(100, 30), Point(350, 80))
    back_btn.setFill("#555960")
    back_btn.draw(win)
    ...
    while True:
        ...
        if inside(click, user_box):
            ...
            identity_text.setText("")
            new_identity = get_input_in_box(win, identity_box)
            if new_identity in ["Student", "Teacher"]:
                user["identity"] = new_identity
                identity_text.setText(new_identity)
            else:
                identity_text.setText(user["identity"])
